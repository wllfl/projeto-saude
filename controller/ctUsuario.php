...
$razao       = (isset($_POST['txtRazao'])) ? strtoupper($_POST['txtRazao']): '';
$cnpj        = (isset($_POST['txtCnpj'])) ? $_POST['txtCnpj'] : '';
$fone        = (isset($_POST['txtFone'])) ? $_POST['txtFone'] : '';
$responsavel = (isset($_POST['txtResponsavel'])) ? strtoupper($_POST['txtResponsavel']): '';
...
$privilegio  = (isset($_POST['cmbPrivilegio'])) ? $_POST['cmbPrivilegio'] : '';
$remenber    = (isset($_POST['ckremenber'])) ? $_POST['ckremenber'] : '';

// Verifica se foi solicitado a validação dos dados para login
if ($acao == 'validar'):
...
// Verifica se foi solicitado a inclusão dos dados
if ($acao == 'incluir'):
    if (!empty($razao) && !empty($cnpj) && !empty($responsavel) && !empty($fone) && !empty($email) && !empty($senha) && !empty($status)):
        $array = array($razao, $cnpj, $fone, $responsavel, $email, base64_encode($senha), $status, $privilegio, 'A');    
...
// Verifica se foi solicitado a edição dos dados
if ($acao == 'editar'):
    if (!empty($razao) && !empty($cnpj) && !empty($responsavel) && !empty($fone) && !empty($email) && !empty($senha) && !empty($status)):
        $array = array($razao, $cnpj, $fone, $responsavel, $email, base64_encode($senha), $status, $privilegio, 'A', $id);
...
// Verifica se foi solicitado a exclusão dos dados
if ($acao == 'excluir'):
    if (!empty($id)):
